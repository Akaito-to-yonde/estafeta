...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  const result = await sendEmail({
    to: notifyEmail,
    subject: 'Reunión confirmada — Estafeta',
    html: wrapHtml(`
      <h1 style="font-size:20px">Reunión confirmada</h1>
      <p>La reunión entre <strong>${speakerEmpresa}</strong> y <strong>${participantEmpresa}</strong> fue confirmada.</p>
      <ul>
    ...
    `),
    meetingId,
    functionName: 'on-meeting-accepted',
  });
