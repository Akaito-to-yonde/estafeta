...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  const { notifyEmail, speakerEmpresa, participantEmpresa, responseNote } = dataOrError;

  const result = await sendEmail({
    to: notifyEmail,
    subject: 'Reunión rechazada — Estafeta',
    html: wrapHtml(`
      <h1 style="font-size:20px">Reunión rechazada</h1>
      <p>La reunión entre <strong>${speakerEmpresa}</strong> y <strong>${participantEmpresa}</strong> fue rechazada.</p>
      ${responseNote ? `<p><strong>Motivo:</strong> ${responseNote}</p>` : ''}
    `),
    meetingId,
    functionName: 'on-meeting-rejected',
  });
