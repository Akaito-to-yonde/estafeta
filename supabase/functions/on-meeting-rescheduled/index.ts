...
async function loadMeetingData(meetingId: number): Promise<MeetingData | Response> {
  ...
  if (meeting) {
    ...
    const resolvedSpeakerEmail = speaker?.email ?? TEST_SPEAKER_EMAIL;
    const resolvedParticipantEmail = participant?.email ?? TEST_PARTICIPANT_EMAIL;
    const notifyEmail =
      meeting.last_updated_by === meeting.speaker_id
        ? resolvedParticipantEmail
        : resolvedSpeakerEmail;
    ...
    return {
      ...
      responseNote: meeting.response_note ?? undefined,
    };
  }

  if (meetingId === TEST_MEETING_ID) {
    return {
      notifyEmail: TEST_PARTICIPANT_EMAIL,
...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  const { notifyEmail, speakerEmpresa, participantEmpresa, start, ending, responseNote } =
    dataOrError;

  const result = await sendEmail({
    to: notifyEmail,
    subject: 'Reunión reagendada — Estafeta',
    html: wrapHtml(`
      <h1 style="font-size:20px">Reunión reagendada</h1>
      <p>La reunión entre <strong>${speakerEmpresa}</strong> y <strong>${participantEmpresa}</strong> fue reagendada.</p>
      <ul>
        <li><strong>Nuevo inicio:</strong> ${start}</li>
        <li><strong>Nuevo fin:</strong> ${ending}</li>
      </ul>
      ${responseNote ? `<p><strong>Nota:</strong> ${responseNote}</p>` : ''}
    `),
    meetingId,
    functionName: 'on-meeting-rescheduled',
  });
